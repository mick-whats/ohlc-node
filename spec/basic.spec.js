...
(function() {
  var _, arrayData, moment, objData, ohlc;
  ...
  test('constructor with object', function() {
    ...
    return expect(pricesArray).toEqual(pricesObject);
  });

  test('constructor throw', function() {
    return expect(function() {
      return ohlc([1, 2, 3, 4]);
    }).toThrowError('inconsistent datatypes');
  });

  test('values(period)', function() {
    var prices;
    expect(arrayData.length).toBe(101);
    prices = ohlc(arrayData).value();
    expect(prices.length).toBe(101);
    prices = ohlc(arrayData).value('week');
    expect(prices.length).toBe(22);
    prices = ohlc(arrayData).value('month');
    return expect(prices.length).toBe(5);
  });

  test('toDaily()', function() {
    var prices;
    prices = ohlc(arrayData).toDaily();
    return expect(prices.find(function(item) {
  ...
  test.skip('toWeekly() with sma', function() {
    var prices;
